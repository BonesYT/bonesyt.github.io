...
function Game() {
    ...
    this.next = EN(200) //progress bar cost
}
...
function barIncrement(id) {
    ...
    if (id == 0) {
        game.points = game.points.add(EN.floor(game.bars[id].points.div(game.bars[id].max)).mul(game.bars[0].pmulti))
        game.tpoints = game.tpoints.add(EN.floor(game.bars[id].points.div(game.bars[id].max)).mul(game.bars[0].pmulti))
    }
