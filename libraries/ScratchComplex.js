class ScratchComplex {
    ...
    complex({r,i}) {
        ...
        if (this.advancedSyntax) {
            const a = x => (x<0?'-':'')+(Math.abs(x)==1?'':Math.abs(x))+'i'
            return r == 0 && i != 0 ? a(i) :
                r < 0 && i > 0 ? a(i) + r :
                r + (i == 0 ? '' : (i > 0 ? '+' : '') + a(i))
        } else {
    ...
    parse(input) {
        ...
        if (!input.includes(',') && this.advancedSyntax) {
            input = input.replace(/\s/g, '').replace(/[eE]\+/g, 'p').replace(/[eE]-/g, 'm')
            let parts = input.split(/(?<!^)[+-]/)

            if (parts.length == 1) {
                const part = parts[0].replace('p','e+').replace('m','e-')
                let a
                return part.endsWith('i') ? {r:0, i: +(a = part.replace(/i$/, ''), a.match(/\d/) ? a : a + '1')} : {r:+part, i:0}
    ...
    ln(x) {
        if (x.i==0) return {r:Math.log(x.r), i:0}
    ...
    complexOperation({oper,z}) {
        z = this.parse(z)
        let r = z.r, i = z.i
        if (['sin','cos','tan'].includes(oper)) r /= 180 / Math.PI, i /= 180 / Math.PI
        let out, alt

        switch (oper) {
            case 'abs': out = [Math.sqrt(r ** 2 + i ** 2), 0]; break
            case 'conjugate': out = [r, -i]; break
            case 'flip': out = [i, r]; break
            case 'floor': out = [Math.floor(r), Math.floor(i)]; break
            case 'ceiling': out = [Math.ceil(r), Math.ceil(i)]; break
            case 'round': out = [Math.round(r), Math.round(i)]; break
            case 'sqrt': out = this.pow(z, {r:.5,i:0}); break
            case 'atan2': out = [Math.atan2(i, r), 0]; break
            case 'e^': out = this.pow({r:Math.E,i:0}, z); break
            case '10^': out = this.pow({r:10,i:0}, z); break
            case 'sign': out = [Math.sign(r), Math.sign(i)]; break
            case 'sin': out = i == 0 ? [Math.sin(r), 0] : [Math.sin(r) * Math.cosh(i), Math.cos(r) * Math.sinh(i)]; break
            case 'cos': out = i == 0 ? [Math.cos(r), 0] : [Math.cos(r) * Math.cosh(i), Math.sin(r) * Math.sinh(i)]; break
            case 'tan': i == 0 ? (out = [Math.tan(r), 0]) : (alt = Math.cos(r*2) + Math.cosh(i*2), out = [Math.sin(r*2) / alt, Math.sinh(i*2) / alt]); break
            case 'asin':
                if (i == 0 && Math.abs(r) <= 1) out = [Math.asin(r) * 180 / Math.PI, 0]
                else {alt =
                    this.mul(this.ln(this.add(this.pow(this.sub({r:1,i:0}, this.pow(z, {r:2,i:0})), {r:.5,i:0}), this.mul(z, {r:0,i:1}))), {r:0,i:-1})
                    out = [alt.r * 180 / Math.PI, alt.i * 180 / Math.PI]
                }    
            ; break
            case 'acos':
                if (i == 0 && Math.abs(r) <= 1) out = [Math.acos(r) * 180 / Math.PI, 0]
                else {alt =
                    this.mul(this.ln(this.add(this.mul(this.pow(this.sub({r:1,i:0}, this.pow(z, {r:2,i:0})), {r:.5,i:0}), {r:0,i:1}), z)), {r:0,i:-1})
                    out = [alt.r * 180 / Math.PI, alt.i * 180 / Math.PI]
                }
            ; break
            case 'atan':
                if (i == 0) out = [Math.atan(r) * 180 / Math.PI, 0]
                    ...
                    this.mul(this.ln(this.div(this.sub({r:1,i:0}, this.mul(z, {r:0,i:1})), this.add({r:1,i:0}, this.mul(z, {r:0,i:1})))), {r:0,i:.5})
                    out = [alt.r * 180 / Math.PI, alt.i * 180 / Math.PI]
                }
            ; break
            case 'ln': alt = this.ln(z), out = [alt.r, alt.i]; break
        }
    ...
    complexLog({n,z}) {
        ...
        const N = {r:Math.log(n.r**2 + n.i**2) / 2, i:Math.atan2(n.i, n.r)}
        const Z = {r:Math.log(z.r**2 + z.i**2) / 2, i:Math.atan2(z.i, z.r)}
    ...
    getInfo() {
        return {
            ...
            blocks: [{
                ...
                blockType: 'reporter',
                text: '[x] + [y]',
                arguments: {x: {type: 'string', defaultValue: ' '}, y: {type: 'string', defaultValue: ' '}}
            }, {
                opcode: 'complexSub',
                blockType: 'reporter',
                text: '[x] - [y]',
                ...
                opcode: 'complexLog',
                blockType: 'reporter',
                text: 'log base [n] of [z]',
                arguments: {n: {type: 'string', defaultValue: '10'}, z: {type: 'string', defaultValue: '100'}}
            }, {
                opcode: 'complexMod',
                blockType: 'reporter',
                text: '[x] mod [y]',
                ...
                blockType: 'command',
                text: 'set syntax mode to [mode]',
                arguments: {mode: {type: 'string', menu: 'complexSyntaxMode'}}
            }, {
                opcode: 'getComplexAdvSyntax',
                blockType: 'Boolean',
                text: 'using advanced syntax?',
                disableMonitor: true
            }],
            menus: {
                complexOper: {
                    acceptReporters: false,
                    items: ['abs', 'conjugate', 'flip', 'floor', 'ceiling', 'round', 'sqrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'atan2', 'ln', 'e^', '10^', 'sign']
                },
                complexSyntaxMode: {
                    acceptReporters: false,
